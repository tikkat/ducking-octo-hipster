...
blocks s = rs ++ transpose rs ++ threeByThreeBlocks
    where
        ...
        threeByThreeBlocks  = [take 9 $ drop n inRightOrder | n <- [0, 9..72]]
            where inRightOrder = concat [take 3 $ drop n row | n <- [0, 3, 6], row <- transpose rs]
...
-- Get current row, coulumn and block in the same list, add number to candidates if not in the list
candidates :: Sudoku -> Pos -> [Int]
candidates s (r, c) = [digit | digit <- [1..9], length bs' == length (delete (Just digit) bs')]
    where
        bs          = blocks s
...
prop_candidates s (r, c) = isSudoku s && isOkay s && r > -1 && r < 9 && c > -1 && c < 9 ==> 
    and [isSudoku (getNewSud cand) && isOkay (getNewSud cand) | cand <- candidates s (r, c)]
    where getNewSud cand = update s (r, c) (Just cand)
...
solve s | not (isSudoku s && isOkay s)  = Nothing
    ...
    where
        solve' :: Sudoku -> Maybe Sudoku
        solve' s    | length allBlanks == 0 = Just s
                    | otherwise             = solve'' s (candidates s choosenBlank) choosenBlank
            where
                allBlanks = blanks s
                choosenBlank = (head allBlanks)

                solve'' :: Sudoku -> [Int] -> Pos -> Maybe Sudoku
                solve'' _ [] _                  = Nothing
                solve'' s (x:xs) choosenBlank   | isJust possibleSolution   = possibleSolution
                                                | otherwise                 = solve'' s xs choosenBlank
                    where possibleSolution = solve' (update s choosenBlank (Just x))
...
readAndSolve f =
    do
        ...
        if isNothing solution
            then
                putStrLn "Found no solution."
            else
                printSudoku (fromJust solution)

-- Make all cells unique with zip, count intersect and compare with number of solved cells in the original
isSolutionOf :: Sudoku -> Sudoku -> Bool
isSolutionOf s1 s2 = isSolved s1 && isOkay s1 && length (rs1 `intersect` rs2) == s2NumSolved
    where
